...
$name = $day = $month = $year = $time = $location = $tags = $notes = $sharew = NULL;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if(($okay_date == 1) && ($okay_name == 1)){
		...
		echo $_POST['sharew'];
		$sharew = $_POST['sharew'];
		addToDatabase($event, $word_month, $day, $year, $time, $location, $tags, $notes, $sharew);
	}

}

function addToDatabase($event, $month, $day, $year, $time, $location, $tags, $notes, $sharew){
	require('connect-db.php');
	require('email.php');

	$user = $_COOKIE['user'];

	$query1 = "SELECT * FROM events ORDER BY summary LIMIT 1";
	$statement1 = $db->prepare($query1);
	$statement1->execute();
	$results1 = $statement1->fetchAll();
	$results1 = $statement1->fetch(PDO::FETCH_ASSOC);
	//echo "<script> console.log(" . $results1 . "); </script>";
	//echo $results1;
	if (!$results1){
		$val = 1;
	}
	else{
		$val = $results1['summary'];
		$val = intval($val);
	}
	...
	$query = "INSERT INTO events (summary, user, event, month, day, year, time, location, tags, notes, sharew)
				VALUES (" . $summary . ",'" . $user . "','" . $event . "','" . $month . "'," . $day . "," . $year . ",'" . $time . "','" . $location . "','" . $tags . "','" . $notes . "','" . $sharew . "');";
	echo $query;
	...
	$statement->bindValue(':sharew', $sharew);
...
<div>
   ...
   <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
	...
	<div class="form-group row col-sm-12">
		...
		<div class="col-sm-10">
			<textarea class="form-control" name="sharew" rows="3" id="sharew" placeholder="Enter emails separated by commas, ex: janedoe@gmail.com, johndoe@yahoo.com"></textarea>
		</div>
	</div>
